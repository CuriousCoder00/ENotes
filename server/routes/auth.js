...
router.post('/createuser', [
        body('name', 'Name shouldn\'t be empty. Please enter valid name.').notEmpty(),
        ...
        try{
            ...
            if(user) {return res.status(400).json({error: "Login credentials error."});}
...
router.post('/login', [
    ...
    try{
        ...
        if(!passwordCompare) {return res.status(400).json({error: "Login credentials error."});}
        // Generating auth token using jwt.sign()
        const authToken = jwt.sign({user:{id: user.id}}, process.env.JWT_SECRET);
        res.json({authToken});
    } catch(error){
